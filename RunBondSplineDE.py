...
import DifferentialEvolution as diffevol
import pandas as pd
...
def runBondSpline(asOfDate, ccyName, bondTypeName, bondCurveName, maturityCutoff, paramEst, onTheRunExcl = 2, offTheRunIncl = 5, estPopulation = None):
	...
	while successFlag == False and numTries < 20:
		try:
			if len(estPopulation) != 0:
				diffEvolSpline = diffevol.differential_evolution_optimizer(fedSplineEvolWrap, population_size=50, f=0.5, cr=0.6, max_iter=600, eps=1e-2, initial_population = estPopulation)
			else:
				diffEvolSpline = diffevol.differential_evolution_optimizer(fedSplineEvolWrap, population_size=50, f=0.5, cr=0.6, max_iter=600, eps=1e-2)
			fedSplineEvolWrap = diffEvolSpline.evaluator
			fitParams = fedSplineEvolWrap.x
			successFlag = True
...
# Get bulk population parameters
def getStoredParamsPopulation(asOfDate, bondTypeName, numRow = 50):
	getPopulation = []
	strSql = """
	...
	for row in cur:
		...
		getPopulation.append([b0,b1,b2,b3,t1,t2])
...
if __name__ == "__main__":
	...
	if len(sys.argv) == 2:
		...
		for index,ccyName in enumerate(ccyNameArray):
			...
			maturityCutoff = asOfDate + timedelta(days=90)

			paramPopulation = getStoredParamsPopulation(asOfDate,bondTypeNameArray[index],30)
			
			if len(paramPopulation) != 0:
				paramEst = paramPopulation[0]
			else:
				paramEst = [0.13,-0.13,0.10,0.10,1.0,2.0]

			paramResult = runBondSpline(asOfDate, ccyName, bondTypeNameArray[index], bondCurveNameArray[index], maturityCutoff, paramEst,onTheRunExclArray[index],offTheRunInclArray[index],paramPopulation)
	...
	if len(sys.argv) == 4:
		...
		paramPopulation = getStoredParamsPopulation(currDate,bondTypeNameArray[currIndex],30)
		...
		while currDate < endDate:
			...
			paramResult = runBondSpline(currDate, ccyName, bondTypeNameArray[currIndex], bondCurveNameArray[currIndex], maturityCutoff, paramEst,onTheRunExclArray[currIndex],offTheRunInclArray[currIndex], paramPopulation)
			# paramEst = [-0.4144769, 0.409366, 36.7183353, 2.3119174458, -1827.1307, 78.787144] # Run the last one [0.1,0.1,0.1,0.1,10,10] [-0.4144769, 0.409366, 36.7183353, 2.3119174458, -1827.1307, 78.787144]
			...
			paramPopulation.insert(0,paramResult)
			paramPopulation.pop()
