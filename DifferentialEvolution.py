...
class differential_evolution_optimizer(object):
	...
	def __init__(self,
		...
		self.eps = eps
		self.init_population_size = 0

		self.initial_population = initial_population

		if self.initial_population is not None:
			assert len(self.initial_population[0]) == self.vector_length
			self.init_population_size = len(self.initial_population)
		else:
			self.init_population_size = 0
		...
		for ii in xrange(self.population_size):
			self.population.append( np.zeros(self.vector_length,'float64') )

		self.scores = np.ones(self.population_size,'float64')*1000
		self.optimize()
		self.best_score = self.scores.min()
		self.best_vector = self.population[np.nanargmin(self.scores)]
	...
	def optimize(self):
		...
		# initialize the population to the init_population if provided
		if self.initial_population is not None:
			for index, initVector in enumerate(self.initial_population):
				self.population[index] = np.array(initVector)
		...
		while not converged:
			self.evolve()
			location = np.nanargmin(self.scores)
		# if self.show_progress:
		...
		#       self.count)
			...
			print str(self.scores.mean()) + " RUN: " + str(self.count)
	...
	# Generate random number parameters based on the constraints.  Generates one parameter at a time
...
class test_rosenbrock_function(object):
	def __init__(self, dim=5):
		...
		self.optimizer =  differential_evolution_optimizer(self,population_size=min(self.n*10,40),n_cross=self.n,cr=0.9, eps=1e-8)
		print list(self.x)
		for x in self.x:
